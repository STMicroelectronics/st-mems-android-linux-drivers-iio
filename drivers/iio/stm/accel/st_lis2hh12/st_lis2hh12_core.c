...
static int lis2hh12_write_max_odr(struct lis2hh12_sensor_data *sdata)
{
	...
	if (max_odr != sdata->cdata->common_odr) {
		...
		sdata->cdata->sensor_deltatime = max_odr ?
...
static int lis2hh12_set_fs(struct lis2hh12_sensor_data *sdata,
			   unsigned int gain)
{
	int err, i;
...
int lis2hh12_update_drdy_irq(struct lis2hh12_sensor_data *sdata, bool state)
{
	...
	return lis2hh12_write_register(sdata->cdata, reg_addr,
				       reg_mask, reg_val);
}
...
int lis2hh12_update_fifo_ths(struct lis2hh12_data *cdata, u8 fifo_len)
{
	...
	err = lis2hh12_write_register(cdata, LIS2HH12_FIFO_THS_ADDR,
				      LIS2HH12_FIFO_THS_MASK, fifo_len);
	if (err < 0)
		return err;
		...
		if (sdata->id == LIS2HH12_ACCEL) {
			...
			*val = *val >> ch->scan_type.shift;
			...
			struct iio_dev *iio_dev_acc =
				  sdata->cdata->iio_sensors_dev[LIS2HH12_ACCEL];
			struct lis2hh12_sensor_data *sdata_acc =
							  iio_priv(iio_dev_acc);
