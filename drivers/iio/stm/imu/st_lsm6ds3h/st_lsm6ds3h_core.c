...
static void st_lsm6ds3h_show_configuration(struct lsm6ds3h_data *cdata)
{
	dev_info(cdata->dev, "- EN BASIC FEATURES: %s\n",
		 IS_ENABLED(CONFIG_IIO_ST_LSM6DS3H_EN_BASIC_FEATURES) ?
		 "enabled" : "disabled");

	dev_info(cdata->dev, "- COUNTER ON DURING SUSPEND: %s\n",
		 IS_ENABLED(CONFIG_ST_LSM6DS3H_STEP_COUNTER_ON_DURING_SUSPEND) ?
		 "enabled" : "disabled");

#if defined(CONFIG_ST_LSM6DS3H_IIO_LIMIT_FIFO) && \
	   (CONFIG_ST_LSM6DS3H_IIO_LIMIT_FIFO > 0)
	dev_info(cdata->dev, "- IIO LIMIT FIFO watermark: %d\n",
		 CONFIG_ST_LSM6DS3H_IIO_LIMIT_FIFO);
#else
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_MASTER_SUPPORT)) {
		...
		dev_info(cdata->dev, "- INTERNAL PULLUP: %s\n",
			 IS_ENABLED(CONFIG_ST_LSM6DS3H_ENABLE_INTERNAL_PULLUP) ?
			 "enabled" : "disabled");
		if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_EXT0_LIS3MDL))
			ext0_str = "LIS3MDL";
		else if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_EXT0_AKM09911))
			ext0_str = "AKM09911";
		else if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_EXT0_AKM09912))
			ext0_str = "AKM09912";
		else if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_EXT0_AKM09916))
			ext0_str = "AKM09916";
		else if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_EXT0_LPS22HB))
			ext0_str = "LPS22HB";
	...
	dev_info(cdata->dev, "- DATA INJECTION: %s\n",
		 IS_ENABLED(CONFIG_ST_LSM6DS3H_XL_DATA_INJECTION) ?
		 "enabled" : "disabled");
}
...
int st_lsm6ds3h_set_fifo_mode(struct lsm6ds3h_data *cdata, enum fifo_mode fm)
{
	...
	case BYPASS:
		...
		break;
	case CONTINUOUS:
		reg_value = ST_LSM6DS3H_FIFO_MODE_CONTINUOUS | ST_LSM6DS3H_FIFO_ODR_MAX;
		break;
	default:
		return -EINVAL;
	}
	...
	if (sdata->cdata->sensors_use_fifo > 0) {
		...
		if (fifo_conf_changed) {
			...
			if ((samples_in_pattern[0] > 0) || (samples_in_pattern[1] > 0) || (samples_in_pattern[2] > 0)) {
				err = st_lsm6ds3h_set_fifo_mode(sdata->cdata, CONTINUOUS);
				if (err < 0)
					goto reenable_fifo_irq;
				...
				if (IS_ENABLED(CONFIG_ST_LSM6DS3H_IIO_MASTER_SUPPORT)) {
					if (((temp_num_samples[2] > 0) &&
						...
						if (sdata->cdata->fifo_enable_timestamp > temp_last_timestamp[2]) {
							...
							for (n = 0; n < n_gen; n++) {
								temp_last_timestamp[2] += temp_deltatime;
								err = st_lsm6ds3h_push_data_with_timestamp(sdata->cdata,
													  ST_MASK_ID_EXT0,
													  sdata->cdata->ext0_last_push,
													  temp_last_timestamp[2]);
								if (err < 0)
...
static int lsm6ds3h_enable_accel(struct lsm6ds3h_data *cdata, enum st_mask_id id, int min_odr)
{
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DS3H_XL_DATA_INJECTION) &&
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DS3H_XL_DATA_INJECTION)) {
		...
		if (sdata->cdata->v_odr[sdata->sindex] != odr) {
			if ((sdata->sindex == ST_MASK_ID_ACCEL) &&
				...
				err = lsm6ds3h_enable_accel(sdata->cdata,
							   ST_MASK_ID_ACCEL,
							   odr);
			else
				err = st_lsm6ds3h_set_odr(sdata, odr, false);
...
#if IS_ENABLED(CONFIG_ST_LSM6DS3H_XL_DATA_INJECTION)
static ssize_t
st_lsm6ds3h_sysfs_set_injection_mode(struct device *dev,
...
int st_lsm6ds3h_probe(struct device *dev, int irq,
		      char *name, struct regmap *regmap)
{
	...
	cdata->fifo_data = devm_kzalloc(dev, ST_LSM6DS3H_MAX_FIFO_SIZE *
	...
	st_hrtimer_setup(&cdata->injection_timer, &st_lsm6ds3h_injection_timer_func);
...
#if IS_ENABLED(CONFIG_IIO_ST_LSM6DS3H_EN_BASIC_FEATURES)
	...
	st_lsm6ds3h_show_configuration(cdata);

	return 0;
}
EXPORT_SYMBOL(st_lsm6ds3h_probe);

static int __maybe_unused st_lsm6ds3h_suspend(struct device *dev)
{
	struct lsm6ds3h_data *cdata = dev_get_drvdata(dev);
	struct lsm6ds3h_sensor_data *sdata;
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		...
		if (IS_ENABLED(CONFIG_ST_LSM6DS3H_STEP_COUNTER_ON_DURING_SUSPEND)) {
...
const struct dev_pm_ops st_lsm6ds3h_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(st_lsm6ds3h_suspend, st_lsm6ds3h_resume)
};
EXPORT_SYMBOL(st_lsm6ds3h_pm_ops);
