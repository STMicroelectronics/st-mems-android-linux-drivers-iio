...
static void st_lsm6dsm_show_configuration(struct lsm6dsm_data *cdata)
{
	dev_info(cdata->dev, "- EN BASIC FEATURES: %s\n",
		 IS_ENABLED(CONFIG_IIO_ST_LSM6DSM_EN_BASIC_FEATURES) ?
		 "enabled" : "disabled");

	dev_info(cdata->dev, "- COUNTER ON DURING SUSPEND: %s\n",
		 IS_ENABLED(CONFIG_ST_LSM6DSM_STEP_COUNTER_ON_DURING_SUSPEND) ?
		 "enabled" : "disabled");

#if defined(CONFIG_ST_LSM6DSM_IIO_LIMIT_FIFO) && \
	   (CONFIG_ST_LSM6DSM_IIO_LIMIT_FIFO > 0)
	dev_info(cdata->dev, "- IIO LIMIT FIFO watermark: %d\n",
		 CONFIG_ST_LSM6DSM_IIO_LIMIT_FIFO);
#else
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_MASTER_SUPPORT)) {
		...
		dev_info(cdata->dev, "- INTERNAL PULLUP: %s\n",
			 IS_ENABLED(CONFIG_ST_LSM6DSM_ENABLE_INTERNAL_PULLUP) ?
			 "enabled" : "disabled");
		if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_LIS3MDL))
			ext0_str = "LIS3MDL";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_AKM09911))
			ext0_str = "AKM09911";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_AKM09912))
			ext0_str = "AKM09912";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_AKM09916))
			ext0_str = "AKM09916";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_LPS22HB))
			ext0_str = "LPS22HB";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_LPS22HH))
			ext0_str = "LPS22HH";
		else if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_EXT0_LIS2MDL))
			ext0_str = "LIS2MDL";
	...
	dev_info(cdata->dev, "- DATA INJECTION: %s\n",
		 IS_ENABLED(CONFIG_ST_LSM6DSM_XL_DATA_INJECTION) ?
		 "enabled" : "disabled");
}
...
static inline int st_lsm6dsm_enable_embedded_page_regs(struct lsm6dsm_data *cdata, bool enable)
{
	...
	return st_lsm6dsm_write_register(cdata,
					 ST_LSM6DSM_FUNC_CFG_ACCESS_ADDR, 1,
					 &value, false);
}
					...
					u8 reg_addr, u8 *data, int len)
{
	...
	err = st_lsm6dsm_write_register(cdata, reg_addr, len, data, false);
	if (err < 0)
...
static int lsm6dsm_set_watermark(struct lsm6dsm_data *cdata)
{
	...
	if (fifo_watermark != cdata->fifo_watermark) {
		...
		err = st_lsm6dsm_write_register(cdata,
						ST_LSM6DSM_FIFO_THR_L_ADDR, 2,
...
int st_lsm6dsm_set_fifo_mode(struct lsm6dsm_data *cdata, enum fifo_mode fm)
{
	...
	err = st_lsm6dsm_write_register(cdata, ST_LSM6DSM_FIFO_MODE_ADDR, 1,
					&reg_value, true);
	if (err < 0)
		return err;
							...
							u8 decimators[3])
{
	...
	return st_lsm6dsm_write_register(cdata, ST_LSM6DSM_FIFO_DECIMATOR_ADDR,
				...
				short new_fifo_decimator[ST_INDIO_DEV_NUM + 1])
{
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DSM_IIO_MASTER_SUPPORT)) {
		if ((accel_decimator == cdata->hwfifo_decimator[ST_MASK_ID_ACCEL]) &&
			...
			if (cdata->fifo_output[ST_MASK_ID_EXT0].decimator !=
					new_fifo_decimator[ST_MASK_ID_EXT0]) {
				return true;
			}
		} else {
			if ((accel_decimator == cdata->hwfifo_decimator[ST_MASK_ID_ACCEL]) &&
					...
					if (((temp_num_samples[2] > 0) && (samples_in_pattern[2] > 0)) &&
						...
						if (sdata->cdata->fifo_enable_timestamp > temp_last_timestamp[2]) {
							...
							for (n = 0; n < n_gen; n++) {
								...
								err = st_lsm6dsm_push_data_with_timestamp(sdata->cdata,
													  ST_MASK_ID_EXT0,
							...
							int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		err = st_lsm6dsm_read_register(sdata->cdata, ch->address,
...
static int st_lsm6dsm_reset_steps(struct lsm6dsm_data *cdata)
{
	...
	err = st_lsm6dsm_read_register(cdata, ST_LSM6DSM_STEP_COUNTER_RES_ADDR,
				       1, &reg_value, true);
	if (err < 0)
		return err;
...
static int st_lsm6dsm_init_sensor(struct lsm6dsm_data *cdata)
{
	...
	err = st_lsm6dsm_write_register(cdata, ST_LSM6DSM_RESET_ADDR, 1,
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DSM_XL_DATA_INJECTION)) {
		...
		if (sdata->cdata->v_odr[sdata->sindex] != odr) {
			if ((sdata->sindex == ST_MASK_ID_ACCEL) &&
				...
				err = lsm6dsm_enable_accel(sdata->cdata,
							   ST_MASK_ID_ACCEL,
							   odr);
			else
				err = st_lsm6dsm_set_odr(sdata, odr, false);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_lsm6dsm_read_register(sdata->cdata, reg_addr, 1,
				       &reg_status, true);
	if (err < 0) {
		mutex_unlock(&sdata->cdata->odr_lock);
		return err;
	...
	err = st_lsm6dsm_write_register(sdata->cdata, reg_addr, 1,
					&temp_reg_status, false);
	if (err < 0) {
		mutex_unlock(&sdata->cdata->odr_lock);
		return err;
	}
	...
	err = st_lsm6dsm_write_register(sdata->cdata, reg_addr,
					1, &reg_status, false);
	if (err < 0) {
		mutex_unlock(&sdata->cdata->odr_lock);
		return err;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_lsm6dsm_write_register(sdata->cdata,
					ST_LSM6DSM_INJECT_XL_X_ADDR,
					3, (u8 *)sample, false);
	if (err < 0) {
		st_iio_device_release_direct(indio_dev);
		return err;
...
int st_lsm6dsm_probe(struct device *dev, int irq,
		     char *name, struct regmap *regmap)
{
	...
	cdata->fifo_data = devm_kzalloc(dev,
					ST_LSM6DSM_MAX_FIFO_SIZE * sizeof(u8),
					GFP_KERNEL);
	if (!cdata->fifo_data)
		return -ENOMEM;
	...
	err = st_lsm6dsm_read_register(cdata, ST_LSM6DSM_WAI_ADDRESS,
				       1, &wai, true);
	if (err < 0) {
	...
	st_lsm6dsm_show_configuration(cdata);
...
static int __maybe_unused st_lsm6dsm_suspend(struct device *dev)
{
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		...
		if (IS_ENABLED(CONFIG_ST_LSM6DSM_STEP_COUNTER_ON_DURING_SUSPEND)) {
