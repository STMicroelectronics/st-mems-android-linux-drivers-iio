...
static void st_ism330dlc_show_configuration(struct ism330dlc_data *cdata)
{
	dev_info(cdata->dev, "- EN BASIC FEATURES: %s\n",
		 IS_ENABLED(CONFIG_IIO_ST_ISM330DLC_EN_BASIC_FEATURES) ?
		 "enabled" : "disabled");

#if defined(CONFIG_ST_ISM330DLC_IIO_LIMIT_FIFO) && \
	   (CONFIG_ST_ISM330DLC_IIO_LIMIT_FIFO > 0)
	dev_info(cdata->dev, "- IIO LIMIT FIFO watermark: %d\n",
		 CONFIG_ST_ISM330DLC_IIO_LIMIT_FIFO);
#else
	...
	if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_MASTER_SUPPORT)) {
		...
		dev_info(cdata->dev, "- INTERNAL PULLUP: %s\n",
			 IS_ENABLED(CONFIG_ST_ISM330DLC_ENABLE_INTERNAL_PULLUP) ?
			 "enabled" : "disabled");
		if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_LIS3MDL))
			ext0_str = "LIS3MDL";
		else if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_AKM09911))
			ext0_str = "AKM09911";
		else if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_AKM09912))
			ext0_str = "AKM09912";
		else if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_AKM09916))
			ext0_str = "AKM09916";
		else if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_LPS22HB))
			ext0_str = "LPS22HB";
		else if (IS_ENABLED(CONFIG_ST_ISM330DLC_IIO_EXT0_LIS2MDL))
			ext0_str = "LIS2MDL";
	...
	dev_info(cdata->dev, "- DATA INJECTION: %s\n",
		 IS_ENABLED(CONFIG_ST_ISM330DLC_XL_DATA_INJECTION) ?
		 "enabled" : "disabled");
}
							 ...
							 bool enable)
{
	...
	return st_ism330dlc_write_register(cdata,
					   ST_ISM330DLC_FUNC_CFG_ACCESS_ADDR,
					   1, &value, false);
}
					 ...
					 u8 reg_addr, u8 *data, int len)
{
	...
	err = st_ism330dlc_write_register(cdata, reg_addr, len, data, false);
	if (err < 0)
...
static int ism330dlc_set_watermark(struct ism330dlc_data *cdata)
{
	...
	if (fifo_watermark != cdata->fifo_watermark) {
		...
		err = st_ism330dlc_write_register(cdata,
						  ST_ISM330DLC_FIFO_THR_L_ADDR,
						  2, (u8 *)&fifo_watermark,
						  true);
		if (err < 0)
			return err;
...
int st_ism330dlc_set_fifo_mode(struct ism330dlc_data *cdata, enum fifo_mode fm)
{
	...
	err = st_ism330dlc_write_register(cdata, ST_ISM330DLC_FIFO_MODE_ADDR,
					  1, &reg_value, true);
	if (err < 0)
		return err;
				      ...
				      u8 decimators[3])
{
	...
	return st_ism330dlc_write_register(cdata,
					   ST_ISM330DLC_FIFO_DECIMATOR_ADDR,
...
static int st_ism330dlc_init_sensor(struct ism330dlc_data *cdata)
{
	...
	err = st_ism330dlc_write_register(cdata, ST_ISM330DLC_RESET_ADDR, 1,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_ism330dlc_read_register(sdata->cdata, reg_addr, 1,
					 &reg_status, true);
	if (err < 0) {
		mutex_unlock(&sdata->cdata->odr_lock);
		return err;
	...
	err = st_ism330dlc_write_register(sdata->cdata, reg_addr,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_ism330dlc_write_register(sdata->cdata,
					  ST_ISM330DLC_INJECT_XL_X_ADDR,
					  3, (u8 *)sample, false);
	if (err < 0) {
		st_iio_device_release_direct(indio_dev);
		return err;
...
int st_ism330dlc_common_probe(struct ism330dlc_data *cdata, int irq)
{
	...
	err = st_ism330dlc_read_register(cdata, ST_ISM330DLC_WAI_ADDRESS,
					 1, &wai, true);
	if (err < 0) {
