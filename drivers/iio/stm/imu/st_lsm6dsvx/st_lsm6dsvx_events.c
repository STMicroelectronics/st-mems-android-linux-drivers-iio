...
/*
 * STMicroelectronics st_lsm6dsvx xl based events function sensor driver
 *
...
#define ST_LSM6DSVX_IS_EVENT_ENABLED(_event_id) (!!(hw->enable_ev_mask & \
						    BIT_ULL(_event_id)))

static struct st_lsm6dsvx_event_t {
	enum st_lsm6dsvx_event_id id;
	char *name;
	u8 irq_mask;
	int req_odr;
	} st_lsm6dsvx_events[] = {
	[ST_LSM6DSVX_EVENT_FF] = {
		.id = ST_LSM6DSVX_EVENT_FF,
		.name = "free_fall",
		.irq_mask = ST_LSM6DSVX_INT_FF_MASK,
		.req_odr = ST_LSM6DSVX_MIN_ODR_IN_FREEFALL,
	},
	[ST_LSM6DSVX_EVENT_WAKEUP] = {
		.id = ST_LSM6DSVX_EVENT_WAKEUP,
		.name = "wake_up",
		.irq_mask = ST_LSM6DSVX_INT_WU_MASK,
		.req_odr = ST_LSM6DSVX_MIN_ODR_IN_WAKEUP,
	},
	[ST_LSM6DSVX_EVENT_6D] = {
		.id = ST_LSM6DSVX_EVENT_6D,
		.name = "sixD",
		.irq_mask = ST_LSM6DSVX_INT_6D_MASK,
		.req_odr = ST_LSM6DSVX_MIN_ODR_IN_6D,
	},

#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
	[ST_LSM6DSVX_EVENT_TAP] = {
		.id = ST_LSM6DSVX_EVENT_TAP,
		.name = "tap",
		.irq_mask = ST_LSM6DSVX_INT_SINGLE_TAP_MASK,
		.req_odr = ST_LSM6DSVX_MIN_ODR_IN_TAP,
	},
	[ST_LSM6DSVX_EVENT_DTAP] = {
		.id = ST_LSM6DSVX_EVENT_DTAP,
		.name = "dtap",
		.irq_mask = ST_LSM6DSVX_INT_DOUBLE_TAP_MASK,
		.req_odr = ST_LSM6DSVX_MIN_ODR_IN_DTAP,
	},
...
static inline bool
st_lsm6dsvx_events_enabled(struct st_lsm6dsvx_hw *hw)
{
	return (ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_FF) ||
		ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_WAKEUP) ||

#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
		ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_TAP) ||
		ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_DTAP) ||
#endif /* LINUX_VERSION_CODE */

		ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_6D));
}

static int st_lsm6dsvx_get_xl_fs(struct st_lsm6dsvx_hw *hw, u8 *xl_fs)
{
	u8 fs_xl_g[] = { 2, 4, 8, 16 }, fs_xl, mul = 1;
	int err;

	if (hw->settings->id.hw_id == ST_LSM6DSV32X_ID)
		mul = 2;

	err = st_lsm6dsvx_read_with_mask(hw,
		hw->settings->fs_table[ST_LSM6DSVX_ID_ACC].reg.addr,
		hw->settings->fs_table[ST_LSM6DSVX_ID_ACC].reg.mask,
		&fs_xl);
	if (err < 0)
		return err;
...
static int st_lsm6dsvx_get_xl_odr(struct st_lsm6dsvx_hw *hw, int *xl_odr)
{
	int i, err;
	u8 odr_xl;
	...
	for (i = 0; i < hw->odr_table[ST_LSM6DSVX_ID_ACC].size; i++) {
		if (odr_xl == hw->odr_table[ST_LSM6DSVX_ID_ACC].odr_avl[i].val)
			break;
	}
	...
	*xl_odr = hw->odr_table[ST_LSM6DSVX_ID_ACC].odr_avl[i].hz;

	return err;
}

static int st_lsm6dsvx_get_default_xl_odr(struct st_lsm6dsvx_hw *hw,
					  enum st_lsm6dsvx_event_id id,
					  int *xl_odr)
{
	int err;
	...
	err = st_lsm6dsvx_get_xl_odr(hw, &odr);
	if (err < 0)
		return err;

	req_odr = st_lsm6dsvx_events[id].req_odr;
...
/*
 * st_lsm6dsvx_set_wake_up_threshold - set wake-up threshold in mg
 ...
 */
static int st_lsm6dsvx_set_wake_up_threshold(struct st_lsm6dsvx_hw *hw,
					    ...
					    int wake_up_duration_ms)
{
	...
	err = st_lsm6dsvx_get_default_xl_odr(hw, ST_LSM6DSVX_EVENT_WAKEUP,
					     &sensor_odr);
	if (err < 0)
		return err;
	...
	err = st_lsm6dsvx_write_with_mask(hw,
					  ST_LSM6DSVX_REG_WAKE_UP_DUR_ADDR,
					  ST_LSM6DSVX_WAKE_DUR_MASK,
					  wake_up_duration);
	if (err < 0)
		return err;
...
/*
 * st_lsm6dsvx_set_freefall_threshold - set free fall threshold detection mg
 * @hw - ST IMU MEMS hw instance
					      ...
					      int freefall_threshold_mg)
{
	...
	for (i = 0; i < ARRAY_SIZE(st_lsm6dsvx_free_fall_threshold); i++) {
		if (freefall_threshold_mg >=
					st_lsm6dsvx_free_fall_threshold[i].mg)
			break;
	}
	...
	err = st_lsm6dsvx_write_with_mask(hw,
					ST_LSM6DSVX_REG_FREE_FALL_ADDR,
					...
					int sixD_threshold_deg)
{
	...
	for (i = 0; i < ARRAY_SIZE(st_lsm6dsvx_6D_threshold); i++) {
		if (sixD_threshold_deg >= st_lsm6dsvx_6D_threshold[i].deg)
			break;
	}
...
#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
/*
 * st_lsm6dsvx_set_tap_threshold - set TAP/DTAP threshold detection in mg
 * @hw - ST IMU MEMS hw instance
 * @tap_threshold_mg - TAP/DTAP threshold in mg
 *
 * th_ug threshold register val = (th_ug * 2 ^ 5) / (1000 * FS_XL)
 */
static int st_lsm6dsvx_set_tap_threshold(struct st_lsm6dsvx_hw *hw,
					 int tap_threshold_mg)
{
	...
	max_th = ST_LSM6DSVX_TAP_THS_X_MASK >>
		  __ffs(ST_LSM6DSVX_TAP_THS_X_MASK);
...
/*
 * st_lsm6dsvx_set_tap_quiet_time - set TAP/DTAP quiet time in ms
 * @hw - ST IMU MEMS hw instance
 * @tap_quiet_time_ms - TAP/DTAP quiet time in LSB
 */
static int st_lsm6dsvx_set_tap_quiet_time(struct st_lsm6dsvx_hw *hw,
					  u8 tap_quiet_time_ms)
{
	u8 tap_quiet_time, max_quite;
	int sensor_odr, err, tmp;

	err = st_lsm6dsvx_get_default_xl_odr(hw, ST_LSM6DSVX_EVENT_TAP,
					     &sensor_odr);
	if (err < 0)
		return err;
	...
	max_quite = ST_LSM6DSVX_QUIET_MASK >> __ffs(ST_LSM6DSVX_QUIET_MASK);
...
/*
 * st_lsm6dsvx_set_tap_shock_time - set TAP/DTAP shock time in ms
 * @hw - ST IMU MEMS hw instance
 * @tap_shock_time_ms - TAP/DTAP shock time in ms
 */
static int st_lsm6dsvx_set_tap_shock_time(struct st_lsm6dsvx_hw *hw,
					  u8 tap_shock_time_ms)
{
	...
	max_tap_shock_time = ST_LSM6DSVX_QUIET_MASK >>
			    __ffs(ST_LSM6DSVX_QUIET_MASK);
...
/*
 * st_lsm6dsvx_set_dtap_duration - set DTAP durationin ms (min time)
 * @hw - ST IMU MEMS hw instance
 * @dtap_duration_ms - DTAP min delay time in ms
 */
static int st_lsm6dsvx_set_dtap_duration(struct st_lsm6dsvx_hw *hw,
					 int dtap_duration_ms)
{
	...
	max_dtap_duration = ST_LSM6DSVX_DUR_MASK >> __ffs(ST_LSM6DSVX_DUR_MASK);
...
int st_lsm6dsvx_read_event_config(struct iio_dev *iio_dev,
	...
	if (chan->type == IIO_ACCEL) {
		...
		case IIO_EV_TYPE_THRESH:
			...
			case IIO_EV_DIR_FALLING:
				return FIELD_GET(BIT(ST_LSM6DSVX_EVENT_FF),
						 hw->enable_ev_mask);

			case IIO_EV_DIR_RISING:
				return FIELD_GET(BIT(ST_LSM6DSVX_EVENT_WAKEUP),
						 hw->enable_ev_mask);
		...
		case IIO_EV_TYPE_CHANGE:
			...
			case IIO_EV_DIR_EITHER:
				return FIELD_GET(BIT(ST_LSM6DSVX_EVENT_6D),
						 hw->enable_ev_mask);
		...
		case IIO_EV_TYPE_GESTURE:
			...
			case IIO_EV_DIR_SINGLETAP:
				return FIELD_GET(BIT(ST_LSM6DSVX_EVENT_TAP),
						 hw->enable_ev_mask);

			case IIO_EV_DIR_DOUBLETAP:
				return FIELD_GET(BIT(ST_LSM6DSVX_EVENT_DTAP),
						 hw->enable_ev_mask);
...
static int
st_lsm6dsvx_check_events_odr_dependency(struct st_lsm6dsvx_hw *hw, int odr,
					enum st_lsm6dsvx_event_id id,
					int enable)
{
	int i, ret = 0;
	...
	for (i = 0; i < ARRAY_SIZE(st_lsm6dsvx_events); i++) {
		current_odr = st_lsm6dsvx_events[i].req_odr;
...
int st_lsm6dsvx_write_event_config(struct iio_dev *iio_dev,
				   const struct iio_chan_spec *chan,
				   enum iio_event_type type,
				   enum iio_event_direction dir,
				   int enable)
{
	...
	err = st_lsm6dsvx_read_locked(hw, int_reg, &int_val, 1);
	if (err < 0)
		return err;

	int_val = st_lsm6dsvx_manipulate_bit(int_val,
					     st_lsm6dsvx_events[id].irq_mask,
					     enable);

	err = st_lsm6dsvx_write_locked(hw, int_reg, int_val);
	if (err < 0)
		return err;
	...
	err = st_lsm6dsvx_write_with_mask(hw,
			      ...
			      !!(int_val & ~ST_LSM6DSVX_REG_INT_EMB_FUNC_MASK));
	if (err < 0)
		return err;

	/* check enabled events ODR dependency */
	req_odr = st_lsm6dsvx_check_events_odr_dependency(hw,
						 st_lsm6dsvx_events[id].req_odr,
						 id, enable);

	err = st_lsm6dsvx_set_odr(iio_priv(hw->iio_devs[ST_LSM6DSVX_ID_ACC]),
				  req_odr, 0);
	if (err < 0)
		return err;
...
int st_lsm6dsvx_write_event_value(struct iio_dev *iio_dev,
				  const struct iio_chan_spec *chan,
				  enum iio_event_type type,
				  enum iio_event_direction dir,
				  enum iio_event_info info,
				  int val, int val2)
{
	...
	case IIO_EV_TYPE_THRESH:
		...
		case IIO_EV_DIR_FALLING:
			...
			case IIO_EV_INFO_VALUE:
				err = st_lsm6dsvx_set_freefall_threshold(hw, val);
				break;
		...
		case IIO_EV_DIR_RISING:
			...
			switch (info) {
			case IIO_EV_INFO_VALUE:
				err = st_lsm6dsvx_set_wake_up_threshold(hw, val);
				break;

			case IIO_EV_INFO_PERIOD:
				err = st_lsm6dsvx_set_wake_up_duration(hw, val);
				break;
	...
	case IIO_EV_TYPE_CHANGE:
		...
		case IIO_EV_DIR_EITHER:
			...
			switch (info) {
			case IIO_EV_INFO_VALUE:
				err = st_lsm6dsvx_set_6D_threshold(hw, val);
				break;
	...
	case IIO_EV_TYPE_GESTURE:
		...
		case IIO_EV_DIR_SINGLETAP:
			switch (info) {
			case IIO_EV_INFO_VALUE:
				err = st_lsm6dsvx_set_tap_threshold(hw, val);
				break;

			case IIO_EV_INFO_RESET_TIMEOUT:
				err = st_lsm6dsvx_set_tap_quiet_time(hw, val);
				break;
		...
		case IIO_EV_DIR_DOUBLETAP:
			...
			case IIO_EV_INFO_TAP2_MIN_DELAY:
				err = st_lsm6dsvx_set_dtap_duration(hw, val);
				break;
...
int st_lsm6dsvx_event_handler(struct st_lsm6dsvx_hw *hw)
{
	...
	if (!st_lsm6dsvx_events_enabled(hw))
		return IRQ_HANDLED;
	...
	if ((reg_src[0] & ST_LSM6DSVX_WAKE_UP_SRC_FF_IA_MASK) &&
	    ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_FF)) {
	...
	if ((reg_src[0] & ST_LSM6DSVX_WAKE_UP_SRC_WU_IA_MASK) &&
	    ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_WAKEUP)) {
		iio_dev = hw->iio_devs[ST_LSM6DSVX_ID_ACC];
		if (reg_src[0] & ST_LSM6DSVX_Z_WU_MASK) {
		...
		if (reg_src[0] & ST_LSM6DSVX_Y_WU_MASK) {
			iio_push_event(iio_dev,
				       IIO_MOD_EVENT_CODE(IIO_ACCEL, 0,
							  IIO_MOD_Y,
							  IIO_EV_TYPE_THRESH,
		...
		if (reg_src[0] & ST_LSM6DSVX_X_WU_MASK) {
			iio_push_event(iio_dev,
				       IIO_MOD_EVENT_CODE(IIO_ACCEL, 0,
							  IIO_MOD_X,
							  IIO_EV_TYPE_THRESH,
	...
	if ((reg_src[2] & ST_LSM6DSVX_D6D_SRC_D6D_IA_MASK) &&
	    ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_6D)) {
		u8 dir;

		iio_dev = hw->iio_devs[ST_LSM6DSVX_ID_ACC];
		if (reg_src[2] & (ST_LSM6DSVX_ZH_MASK | ST_LSM6DSVX_ZL_MASK)) {
			dir = (reg_src[2] & ST_LSM6DSVX_ZH_MASK) ?
		...
		if (reg_src[2] & (ST_LSM6DSVX_YH_MASK | ST_LSM6DSVX_YL_MASK)) {
			dir = (reg_src[2] & ST_LSM6DSVX_YH_MASK) ?
			      IIO_EV_DIR_RISING : IIO_EV_DIR_FALLING;
			iio_push_event(iio_dev,
				       IIO_MOD_EVENT_CODE(IIO_ACCEL, 0,
							  IIO_MOD_Y,
		...
		if (reg_src[2] & (ST_LSM6DSVX_XH_MASK | ST_LSM6DSVX_XL_MASK)) {
			dir = (reg_src[2] & ST_LSM6DSVX_XH_MASK) ?
			      IIO_EV_DIR_RISING : IIO_EV_DIR_FALLING;
			iio_push_event(iio_dev,
				       IIO_MOD_EVENT_CODE(IIO_ACCEL, 0,
							  IIO_MOD_X,
...
#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
	if ((reg_src[1] & ST_LSM6DSVX_SINGLE_TAP_IA_MASK) &&
	    ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_TAP)) {
		u8 mod;

		iio_dev = hw->iio_devs[ST_LSM6DSVX_ID_ACC];
		if (reg_src[1] & ST_LSM6DSVX_X_TAP_MASK)
			mod = IIO_MOD_X;
		else if (reg_src[1] & ST_LSM6DSVX_Y_TAP_MASK)
			mod = IIO_MOD_Y;
		else if (reg_src[1] & ST_LSM6DSVX_Z_TAP_MASK)
			mod = IIO_MOD_Z;
		else
			return IRQ_HANDLED;
	...
	if ((reg_src[1] & ST_LSM6DSVX_DOUBLE_TAP_IA_MASK) &&
	    ST_LSM6DSVX_IS_EVENT_ENABLED(ST_LSM6DSVX_EVENT_DTAP)) {
		u8 mod;
...
int st_lsm6dsvx_update_threshold_events(struct st_lsm6dsvx_hw *hw)
{
	...
	ret = st_lsm6dsvx_set_wake_up_threshold(hw, hw->wk_th_mg);
	if (ret < 0)
		return ret;

#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
	ret = st_lsm6dsvx_set_tap_threshold(hw, hw->tap_threshold);
	if (ret < 0)
		return ret;
#endif /* LINUX_VERSION_CODE */
...
int st_lsm6dsvx_update_duration_events(struct st_lsm6dsvx_hw *hw)
{
	...
	ret = st_lsm6dsvx_set_wake_up_duration(hw, hw->wk_dur_ms);
	if (ret < 0)
		return ret;

#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
	ret = st_lsm6dsvx_set_tap_quiet_time(hw, hw->tap_quiet_time);
	if (ret < 0)
		return ret;

	ret = st_lsm6dsvx_set_dtap_duration(hw, hw->dtap_duration);
	if (ret < 0)
		return ret;

	ret = st_lsm6dsvx_set_tap_shock_time(hw, hw->tap_shock_time);
	if (ret < 0)
		return ret;
#endif /* LINUX_VERSION_CODE */
 ...
 * 			   IIO attribute)
 */
int st_lsm6dsvx_event_init(struct st_lsm6dsvx_hw *hw)
{
	int err;

	err = regmap_update_bits(hw->regmap, ST_LSM6DSVX_REG_TAP_CFG0_ADDR,
				 ST_LSM6DSVX_TAP_SLOPE_FDS_MASK,
				 FIELD_PREP(ST_LSM6DSVX_TAP_SLOPE_FDS_MASK, 1));
	if (err < 0)
		return err;

	/* Set default wake-up threshold to 100 mg */
	err = st_lsm6dsvx_set_wake_up_threshold(hw, 100);
	if (err < 0)
		return err;
	...
	err = st_lsm6dsvx_set_tap_threshold(hw, 570);
	if (err < 0)
		return err;

	/* setting default tap/dtap quiet time to 10 ms */
	err = st_lsm6dsvx_set_tap_quiet_time(hw, 10);
	if (err < 0)
		return err;

	/* setting default dtap min delay time to 540 ms */
	err = st_lsm6dsvx_set_dtap_duration(hw, 540);
	if (err < 0)
		return err;

	/* setting default tap/dtap shock time to 40 ms */
	err = st_lsm6dsvx_set_tap_shock_time(hw, 40);
	if (err < 0)
		return err;
