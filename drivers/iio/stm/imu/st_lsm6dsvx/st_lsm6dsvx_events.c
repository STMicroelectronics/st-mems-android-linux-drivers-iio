...
/*
 * st_lsm6dsvx_set_wake_up_thershold - set wake-up threshold in mg
				  ...
				  int val, int val2)
{
	...
	case IIO_EV_TYPE_THRESH:
		...
		case IIO_EV_DIR_RISING:
			...
			switch (info) {
			case IIO_EV_INFO_VALUE:
				err = st_lsm6dsvx_set_wake_up_thershold(hw, val);
				break;
...
int st_lsm6dsvx_update_threshold_events(struct st_lsm6dsvx_hw *hw)
{
	...
	ret = st_lsm6dsvx_set_wake_up_thershold(hw, hw->wk_th_mg);
	if (ret < 0)
		return ret;
...
int st_lsm6dsvx_event_init(struct st_lsm6dsvx_hw *hw)
{
	...
	err = st_lsm6dsvx_set_wake_up_thershold(hw, 100);
	if (err < 0)
		return err;
