...
#define ST_LSM6DSVX_STEP_CHANNEL(addr)					\
{									\
	.type = IIO_STEPS,						\
	.address = addr,						\
	...
	.event_spec = st_lsm6dsvx_step_events,				\
	.num_event_specs = ARRAY_SIZE(st_lsm6dsvx_step_events),		\
}

static const struct iio_event_spec st_lsm6dsvx_step_events[] = {
	{
		/* step detector */
...
static const struct iio_chan_spec st_lsm6dsvx_step_channels[] = {
	ST_LSM6DSVX_STEP_CHANNEL(ST_LSM6DSVX_REG_STEP_COUNTER_L_ADDR),
...
static int st_lsm6dsvx_reset_step_counter(struct iio_dev *iio_dev)
{
	...
	err = __st_lsm6dsvx_write_with_mask(hw,
					    ...
					    ST_LSM6DSVX_PEDO_RST_STEP_MASK,
					    1);
 ...
 * @return  < 0 if error, 0 otherwise
 */
static int st_lsm6dsvx_pedometer_enable(struct st_lsm6dsvx_sensor *sensor,
					bool enable)
{
	struct st_lsm6dsvx_hw *hw = sensor->hw;
	int err;
 ...
 */
static int
st_lsm6dsvx_step_event_enable(struct st_lsm6dsvx_sensor *sensor, bool enable)
{
	struct st_lsm6dsvx_hw *hw = sensor->hw;
	int err;

	err = st_lsm6dsvx_pedometer_enable(sensor, enable);
	if (err < 0)
		return err;
	...
	err = __st_lsm6dsvx_write_with_mask(hw, hw->embfunc_irq_reg,
 ...
 * @return  < 0 if error, 0 otherwise
 */
static int st_lsm6dsvx_signmot_enable(struct st_lsm6dsvx_sensor *sensor,
				      bool enable)
{
	struct st_lsm6dsvx_hw *hw = sensor->hw;
	int err;
 ...
 */
static int
st_lsm6dsvx_signmot_event_enable(struct st_lsm6dsvx_sensor *sensor, bool enable)
{
	struct st_lsm6dsvx_hw *hw = sensor->hw;
	int err;

	err = st_lsm6dsvx_signmot_enable(sensor, enable);
	if (err < 0)
		return err;
...
static int st_lsm6dsvx_read_step_counter(struct st_lsm6dsvx_sensor *sensor,
					 u8 addr, int *val)
{
	struct st_lsm6dsvx_hw *hw = sensor->hw;
	__le16 data;
 ...
 */
static int st_lsm6dsvx_read_event_step_config(struct iio_dev *iio_dev,
 ...
 */
static int st_lsm6dsvx_write_event_step_config(struct iio_dev *iio_dev,
					       const struct iio_chan_spec *chan,
					       enum iio_event_type type,
					       enum iio_event_direction dir,
					       int state)
{
	...
	if (chan->type == IIO_STEPS) {
		...
		case IIO_EV_TYPE_CHANGE:
			 ...
			 */
			case IIO_EV_DIR_NONE:
				err = st_lsm6dsvx_step_event_enable(sensor,
								    state);
				if (state)
					hw->enable_ev_mask |= BIT(ST_LSM6DSVX_EVENT_STEPC);
				else
					hw->enable_ev_mask &= ~BIT(ST_LSM6DSVX_EVENT_STEPC);
			 ...
			 */
			case IIO_EV_DIR_RISING:
				err = st_lsm6dsvx_signmot_event_enable(sensor,
								       state);
				if (state)
					hw->enable_ev_mask |= BIT(ST_LSM6DSVX_EVENT_SIGNMOT);
				else
					hw->enable_ev_mask &= ~BIT(ST_LSM6DSVX_EVENT_SIGNMOT);
				break;
...
static int st_lsm6dsvx_read_step_raw(struct iio_dev *iio_dev,
				     ...
				     int *val, int *val2, long mask)
{
	...
	case IIO_STEPS:
		...
		case IIO_CHAN_INFO_PROCESSED:
			...
			ret = st_lsm6dsvx_read_step_counter(sensor,
							    ch->address, val);
			iio_device_release_direct_mode(iio_dev);
			return IIO_VAL_INT;
...
static int st_lsm6dsvx_write_step_raw(struct iio_dev *iio_dev,
				      struct iio_chan_spec const *ch,
				      int val, int val2, long mask)
{
	struct st_lsm6dsvx_sensor *sensor = iio_priv(iio_dev);
	...
	if ((ch->type == IIO_STEPS) &&
		...
		return st_lsm6dsvx_step_enable(sensor, val);
...
static IIO_DEVICE_ATTR(module_id, 0444,
		       st_lsm6dsvx_get_module_id, NULL, 0);

static struct attribute *st_lsm6dsvx_step_attributes[] = {
	&iio_dev_attr_reset_counter.dev_attr.attr,
	&iio_dev_attr_module_id.dev_attr.attr,
	NULL,
};

static const struct attribute_group st_lsm6dsvx_step_attribute_group = {
	.attrs = st_lsm6dsvx_step_attributes,
};

static const struct iio_info st_lsm6dsvx_step_info = {
	.attrs = &st_lsm6dsvx_step_attribute_group,
	.read_raw = st_lsm6dsvx_read_step_raw,
	.write_raw = st_lsm6dsvx_write_step_raw,
	.read_event_config = st_lsm6dsvx_read_event_step_config,
	.write_event_config = st_lsm6dsvx_write_event_step_config,
};

/**
 * st_lsm6dsvx_embfunc_handler_thread() - Bottom handler for embedded function
 *
...
int st_lsm6dsvx_embfunc_handler_thread(struct st_lsm6dsvx_hw *hw)
{
	...
	if (!(hw->enable_mask & BIT(ST_LSM6DSVX_ID_STEP_COUNTER)))
		return IRQ_HANDLED;
...
static struct
iio_dev *st_lsm6dsvx_alloc_step_iiodev(struct st_lsm6dsvx_hw *hw,
				       enum st_lsm6dsvx_sensor_id id)
{
	...
	iio_dev->channels = st_lsm6dsvx_step_channels;
	iio_dev->num_channels =
			ARRAY_SIZE(st_lsm6dsvx_step_channels);
	scnprintf(sensor->name, sizeof(sensor->name),
		  "%s_step", hw->settings->id.name);
	iio_dev->info = &st_lsm6dsvx_step_info;

	/* request acc ODR to works properly */
	sensor->odr = ST_LSM6DSVX_MIN_ODR_IN_EMB_FUNC;
	sensor->uodr = 0;
 ...
 * @return  < 0 if error, 0 otherwise
 */
static int st_lsm6dsvx_embedded_function_init(struct st_lsm6dsvx_hw *hw)
{
	int err;
	...
	__st_lsm6dsvx_write_with_mask(hw, ST_LSM6DSVX_REG_PAGE_RW_ADDR,
...
/**
 * st_lsm6dsvx_embfunc_probe() - Enable embedded functionalities
 *
 * @hw: ST IMU MEMS hw instance.
 *
...
int st_lsm6dsvx_embfunc_probe(struct st_lsm6dsvx_hw *hw)
{
	...
	id = ST_LSM6DSVX_ID_STEP_COUNTER;
	hw->iio_devs[id] = st_lsm6dsvx_alloc_step_iiodev(hw, id);
	if (!hw->iio_devs[id])
		return -ENOMEM;

	return st_lsm6dsvx_embedded_function_init(hw);
}
