...
#define ST_LSM6DSVX_TSYNC_DECREMENT(_id)	do { \
...
static void st_lsm6dsvx_read_hw_timestamp(struct st_lsm6dsvx_hw *hw)
{
	...
	ST_LSM6DSVX_TSYNC_DECREMENT(ST_LSM6DSVX_ID_GYRO);
	ST_LSM6DSVX_TSYNC_DECREMENT(ST_LSM6DSVX_ID_ACC);
	ST_LSM6DSVX_TSYNC_DECREMENT(ST_LSM6DSVX_ID_TEMP);
	ST_LSM6DSVX_TSYNC_DECREMENT(ST_LSM6DSVX_ID_EXT0);
	ST_LSM6DSVX_TSYNC_DECREMENT(ST_LSM6DSVX_ID_EXT1);

	if ((hw->timesync_c[ST_LSM6DSVX_ID_GYRO] == 0) &&
	    (hw->timesync_c[ST_LSM6DSVX_ID_ACC] == 0) &&
	    (hw->timesync_c[ST_LSM6DSVX_ID_TEMP] == 0) &&
	    (hw->timesync_c[ST_LSM6DSVX_ID_EXT0] == 0) &&
	    (hw->timesync_c[ST_LSM6DSVX_ID_EXT1] == 0)) {
	...
	if (hw->enable_mask & BIT_ULL(ST_LSM6DSVX_ID_EXT0)) {
		iio_push_event(hw->iio_devs[ST_LSM6DSVX_ID_EXT0], eventLSB,
			       timestamp_cpu);
		iio_push_event(hw->iio_devs[ST_LSM6DSVX_ID_EXT0], eventMSB,
			       timestamp_cpu);
	}
	if (hw->enable_mask & BIT_ULL(ST_LSM6DSVX_ID_EXT1)) {
		iio_push_event(hw->iio_devs[ST_LSM6DSVX_ID_EXT1], eventLSB,
			       timestamp_cpu);
		iio_push_event(hw->iio_devs[ST_LSM6DSVX_ID_EXT1], eventMSB,
			       timestamp_cpu);
	}
}
...
int st_lsm6dsvx_hwtimesync_init(struct st_lsm6dsvx_hw *hw)
{
	...
	st_hrtimer_setup(&hw->timesync_timer, st_lsm6dsvx_timer_fn);
