...
/*
 * STMicroelectronics st_lsm6dsvxhg sensor hub library driver
 *
...
#define ST_LSM6DSVXHG_MAX_SLV_NUM			2

/**
 * @struct  st_lsm6dsvxhg_ext_pwr
...
/**
 * @struct  st_lsm6dsvxhg_ext_dev_settings
 ...
 */
static struct st_lsm6dsvxhg_ext_dev_settings {
	u8 i2c_addr[2];
	u8 wai_addr;
	u8 wai_val;
	struct st_lsm6dsvxhg_odr_table_entry odr_table;
	struct st_lsm6dsvxhg_fs_table_entry fs_table;
	struct st_lsm6dsvxhg_reg temp_comp_reg;
	struct st_lsm6dsvxhg_ext_pwr pwr_table;
	struct st_lsm6dsvxhg_reg off_canc_reg;
	struct st_lsm6dsvxhg_reg bdu_reg;
	unsigned long ext_available_scan_masks[2];

#if defined(CONFIG_IIO_ST_LSM6DSVXHG_ASYNC_HW_TIMESTAMP)
	...
	/* LIS2MDL */
	{
		...
		.ext_channels[0] = ST_LSM6DSVXHG_DATA_CHANNEL(IIO_MAGN, 0x68,
							   1, IIO_MOD_X, 0,
							   16, 16, 's', NULL),
		.ext_channels[1] = ST_LSM6DSVXHG_DATA_CHANNEL(IIO_MAGN, 0x6a,
							   1, IIO_MOD_Y, 1,
							   16, 16, 's', NULL),
		.ext_channels[2] = ST_LSM6DSVXHG_DATA_CHANNEL(IIO_MAGN, 0x6c,
							   1, IIO_MOD_Z, 2,
							   16, 16, 's', NULL),
		.ext_channels[3] = ST_LSM6DSVXHG_EVENT_CHANNEL(IIO_MAGN, flush),
	...
	/* LPS22HH */
	{
		...
		.odr_table = {
			...
			.odr_avl[0] = {   1, 10,  0x1,  0x0 },
			.odr_avl[1] = {  10,  0,  0x2,  0x0 },
			.odr_avl[2] = {  25,  0,  0x3,  0x0 },
			.odr_avl[3] = {  50,  0,  0x4,  0x0 },
			.odr_avl[4] = { 100,  0,  0x6,  0x0 },
		},
		.fs_table = {
			.size = 1,
			/* hPa micro scale */
		...
		.ext_channels[0] = ST_LSM6DSVXHG_DATA_CHANNEL(IIO_PRESSURE, 0x28,
							    0, IIO_NO_MOD, 0,
							    24, 32, 'u', NULL),
		.ext_channels[1] = ST_LSM6DSVXHG_EVENT_CHANNEL(IIO_PRESSURE,
							    flush),
	...
	},
	{
		...
		.ext_channels[0] = ST_LSM6DSVXHG_DATA_CHANNEL(IIO_PRESSURE,
							 0x28, 0, IIO_NO_MOD, 0,
							 24, 32, 'u', NULL),
		.ext_channels[1] = ST_LSM6DSVXHG_EVENT_CHANNEL(IIO_PRESSURE,
							       flush),
 ...
 */
static inline
void st_lsm6dsvxhg_shub_wait_complete(struct st_lsm6dsvxhg_hw *hw,
				      struct st_lsm6dsvxhg_sensor *sensor)
{
	...
	accel_odr = hw->odr_table[ST_LSM6DSVXHG_ID_ACC].odr_avl[0].hz;
	odr = (hw->enable_mask & BIT(ST_LSM6DSVXHG_ID_ACC)) ?
	...
	do {
		...
		regmap_bulk_read(hw->regmap,
				 ST_LSM6DSVXHG_STATUS_MASTER_MAINPAGE_ADDR,
				 (unsigned int *)&data, 1);

		if (data & ST_LSM6DSVXHG_SENS_HUB_ENDOP_MASK)
			break;
 ...
 */
static int st_lsm6dsvxhg_shub_read_reg(struct st_lsm6dsvxhg_hw *hw, u8 addr,
				       u8 *data, int len)
{
	int err;
	...
	err = st_lsm6dsvxhg_set_page_access(hw,
					    ST_LSM6DSVXHG_SHUB_REG_ACCESS_MASK,
					    1);
	if (err < 0)
		goto out;
	...
	st_lsm6dsvxhg_set_page_access(hw, ST_LSM6DSVXHG_SHUB_REG_ACCESS_MASK,
				      0);
out:
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dsvxhg_shub_write_reg(struct st_lsm6dsvxhg_hw *hw,
 ...
 */
static int
st_lsm6dsvxhg_shub_master_enable(struct st_lsm6dsvxhg_sensor *sensor,
				 bool enable)
{
	...
	err = __st_lsm6dsvxhg_write_with_mask(hw,
				      ST_LSM6DSVXHG_CONTROLLER_CONFIG_ADDR,
				      ST_LSM6DSVXHG_CONTROLLER_ON_MASK, 1);

	err = __st_lsm6dsvxhg_write_with_mask(hw,
				     ST_LSM6DSVXHG_TGT0_CONFIG_ADDR,
				     ST_LSM6DSVXHG_SHUB_ODR_MASK,
				     enable == 1 ?
				     ST_LSM6DSVXHG_SHUB_ODR_120HZ_VAL : 0);
...
int st_lsm6dsvxhg_shub_read(struct st_lsm6dsvxhg_sensor *sensor,
			    u8 addr, u8 *data, int len)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dsvxhg_hw *hw = sensor->hw;
	u8 out_addr = ST_LSM6DSVXHG_SENSOR_HUB_1_ADDR + hw->ext_data_len;
	u8 config[3];
	int err;
	...
	err = st_lsm6dsvxhg_shub_write_reg(hw, ST_LSM6DSVXHG_TGT3_ADD_ADDR,
					   config, sizeof(config));
	if (err < 0)
		return err;

	err = st_lsm6dsvxhg_shub_master_enable(sensor, true);
	if (err < 0)
		return err;

	st_lsm6dsvxhg_shub_wait_complete(hw, sensor);

	err = st_lsm6dsvxhg_shub_read_reg(hw, out_addr, data, len & 0x7);

	st_lsm6dsvxhg_shub_master_enable(sensor, false);

	memset(config, 0, sizeof(config));
	return st_lsm6dsvxhg_shub_write_reg(hw, ST_LSM6DSVXHG_TGT3_ADD_ADDR,
					    config, sizeof(config));
}
 ...
 * @return  0 if OK, < 0 if ERROR
 */
static int st_lsm6dsvxhg_shub_write(struct st_lsm6dsvxhg_sensor *sensor,
				    u8 addr, u8 *data, int len)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dsvxhg_hw *hw = sensor->hw;
	u8 mconfig = ST_LSM6DSVXHG_WRITE_ONCE_MASK | 3;
	u8 config[3] = { 0, 0, 0x80 };
	int err, i;

	/* AuxSens = 3 + wr once */
	err = st_lsm6dsvxhg_shub_write_reg(hw,
					   ST_LSM6DSVXHG_CONTROLLER_CONFIG_ADDR,
	...
	for (i = 0; i < len; i++) {
		...
		err = st_lsm6dsvxhg_shub_write_reg(hw,
						   ST_LSM6DSVXHG_TGT0_ADD_ADDR,
						   config, sizeof(config));
		if (err < 0)
			return err;

		err = st_lsm6dsvxhg_shub_write_reg(hw,
					      ST_LSM6DSVXHG_DATAWRITE_SLV0_ADDR,
					      &data[i], 1);
		if (err < 0)
			return err;
	...
	return st_lsm6dsvxhg_shub_write_reg(hw, ST_LSM6DSVXHG_TGT0_ADD_ADDR,
					    config, sizeof(config));
}
 ...
 */
static int
st_lsm6dsvxhg_shub_write_with_mask(struct st_lsm6dsvxhg_sensor *sensor,
				   u8 addr, u8 mask, u8 val)
{
	int err;
	u8 data;

	err = st_lsm6dsvxhg_shub_read(sensor, addr, &data, sizeof(data));
	if (err < 0)
		return err;

	data = ((data & ~mask) | (val << __ffs(mask) & mask));

	return st_lsm6dsvxhg_shub_write(sensor, addr, &data, sizeof(data));
}
 ...
 */
static int
st_lsm6dsvxhg_shub_config_channels(struct st_lsm6dsvxhg_sensor *sensor,
				   bool enable)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info;
	struct st_lsm6dsvxhg_hw *hw = sensor->hw;
	...
	for (i = ST_LSM6DSVXHG_ID_EXT0; i <= ST_LSM6DSVXHG_ID_EXT1; i++) {
		if (!hw->iio_devs[i])
			continue;
		...
		config[j + 2] = ST_LSM6DSVXHG_BATCH_EXT_SENS_EN_MASK |
				(ext_info->ext_dev_settings->data_len &
				 ST_LSM6DSVXHG_SLAVE_NUMOP_MASK);
		j += 3;
	}

	return st_lsm6dsvxhg_shub_write_reg(hw, ST_LSM6DSVXHG_TGT1_ADD_ADDR,
					    config, sizeof(config));
}
 ...
 */
static int
st_lsm6dsvxhg_shub_get_odr_val(struct st_lsm6dsvxhg_sensor *sensor,
			       u16 odr, u8 *val)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info = &sensor->ext_dev_info;
	int i;
 ...
 * @return  0 if OK, negative value for ERROR
 */
static int st_lsm6dsvxhg_shub_set_odr(struct st_lsm6dsvxhg_sensor *sensor,
				      u16 odr)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info = &sensor->ext_dev_info;
	struct st_lsm6dsvxhg_hw *hw = sensor->hw;
	...
	err = st_lsm6dsvxhg_shub_get_odr_val(sensor, odr, &odr_val);
	if (err < 0)
		return err;
	...
	return st_lsm6dsvxhg_shub_write_with_mask(sensor,
				  ...
				  bool enable)
{
	...
	if (sensor->hw->has_hw_fifo) {
		err = st_lsm6dsvxhg_shub_config_channels(sensor, enable);
		if (err < 0)
			return err;
	}

	if (enable) {
		err = st_lsm6dsvxhg_shub_set_odr(sensor, sensor->odr);
		if (err < 0)
			return err;
	} else {
		err = st_lsm6dsvxhg_shub_write_with_mask(sensor,
			 ext_info->ext_dev_settings->odr_table.reg.addr,
	...
	return st_lsm6dsvxhg_shub_master_enable(sensor, enable);
}

static inline u32 st_lsm6dsvxhg_get_unaligned_le24(const u8 *p)
{
 ...
 */
static int
st_lsm6dsvxhg_shub_read_oneshot(struct st_lsm6dsvxhg_sensor *sensor,
				struct iio_chan_spec const *ch, int *val)
{
	int err, delay, len = ch->scan_type.realbits >> 3;
	u8 data[4];

	err = st_lsm6dsvxhg_shub_set_enable(sensor, true);
	if (err < 0)
		return err;
	...
	err = st_lsm6dsvxhg_shub_read(sensor, ch->address, data, len);
	if (err < 0)
		return err;

	st_lsm6dsvxhg_shub_set_enable(sensor, false);

	switch (len) {
	case 3:
		*val = (s32)st_lsm6dsvxhg_get_unaligned_le24(data);
		break;
	case 2:
 ...
 */
static int st_lsm6dsvxhg_shub_read_raw(struct iio_dev *iio_dev,
				       ...
				       int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		ret = st_lsm6dsvxhg_shub_read_oneshot(sensor, ch, val);
 ...
 * @return  0 if OK, -EINVAL value for ERROR
 */
static int st_lsm6dsvxhg_shub_write_raw(struct iio_dev *iio_dev,
					...
					int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SAMP_FREQ: {
		...
		err = st_lsm6dsvxhg_shub_get_odr_val(sensor, val, &data);
		if (!err)
			sensor->odr = val;
		break;
	}
 ...
 */
static ssize_t
st_lsm6dsvxhg_sysfs_shub_sampling_freq_avail(struct device *dev,
					     ...
					     char *buf)
{
	...
	for (i = 0; i < ST_LSM6DSVXHG_ODR_LIST_SIZE; i++) {
 ...
 */
static ssize_t
st_lsm6dsvxhg_sysfs_shub_scale_avail(struct device *dev,
...
static IIO_DEV_ATTR_SAMP_FREQ_AVAIL(st_lsm6dsvxhg_sysfs_shub_sampling_freq_avail);
static IIO_DEVICE_ATTR(in_ext_scale_available, 0444,
		       st_lsm6dsvxhg_sysfs_shub_scale_avail, NULL, 0);
static IIO_DEVICE_ATTR(hwfifo_watermark_max, 0444,
		       st_lsm6dsvxhg_get_max_watermark, NULL, 0);
static IIO_DEVICE_ATTR(hwfifo_flush, 0200, NULL, st_lsm6dsvxhg_flush_fifo, 0);
static IIO_DEVICE_ATTR(hwfifo_watermark, 0644, st_lsm6dsvxhg_get_watermark,
		       st_lsm6dsvxhg_set_watermark, 0);
static IIO_DEVICE_ATTR(module_id, 0444, st_lsm6dsvxhg_get_module_id, NULL, 0);

static struct attribute *st_lsm6dsvxhg_ext_attributes[] = {
...
static const struct attribute_group st_lsm6dsvxhg_ext_attribute_group = {
	.attrs = st_lsm6dsvxhg_ext_attributes,
};

static const struct iio_info st_lsm6dsvxhg_ext_info = {
	.attrs = &st_lsm6dsvxhg_ext_attribute_group,
	.read_raw = st_lsm6dsvxhg_shub_read_raw,
	.write_raw = st_lsm6dsvxhg_shub_write_raw,
};
...
st_lsm6dsvxhg_shub_alloc_iio_dev(struct st_lsm6dsvxhg_hw *hw,
		const struct st_lsm6dsvxhg_ext_dev_settings *ext_settings,
		enum st_lsm6dsvxhg_sensor_id id, u8 i2c_addr)
{
	struct st_lsm6dsvxhg_sensor *sensor;
	...
	iio_dev->info = &st_lsm6dsvxhg_ext_info;
...
static int
st_lsm6dsvxhg_shub_init_remote_sensor(struct st_lsm6dsvxhg_sensor *sensor)
{
	struct st_lsm6dsvxhg_ext_dev_info *ext_info = &sensor->ext_dev_info;
	int err = 0;
...
{
	const struct st_lsm6dsvxhg_ext_dev_settings *settings;
	struct st_lsm6dsvxhg_sensor *acc_sensor, *sensor;
	...
	acc_sensor->odr = hw->odr_table[ST_LSM6DSVXHG_ID_ACC].odr_avl[4].hz;

	while (i < ARRAY_SIZE(st_lsm6dsvxhg_ext_dev_table) &&
	       num_ext_dev < ST_LSM6DSVXHG_MAX_SLV_NUM) {
		settings = &st_lsm6dsvxhg_ext_dev_table[i];
		for (j = 0; j < ARRAY_SIZE(settings->i2c_addr); j++) {
			...
			err = st_lsm6dsvxhg_shub_master_enable(acc_sensor,
							       true);
			if (err < 0)
				return err;

			st_lsm6dsvxhg_shub_wait_complete(hw, acc_sensor);

			err = st_lsm6dsvxhg_shub_read_reg(hw,
						ST_LSM6DSVXHG_SENSOR_HUB_1_ADDR,
						&data, sizeof(data));

			st_lsm6dsvxhg_shub_master_enable(acc_sensor, false);
			...
			id = ST_LSM6DSVXHG_ID_EXT0 + num_ext_dev;
			hw->iio_devs[id] = st_lsm6dsvxhg_shub_alloc_iio_dev(hw,
							settings, id,
							settings->i2c_addr[j]);
			if (!hw->iio_devs[id])
				return -ENOMEM;
			...
			err = st_lsm6dsvxhg_shub_init_remote_sensor(sensor);
			if (err < 0)
				return err;
	...
	memset(config, 0, sizeof(config));
	err = st_lsm6dsvxhg_shub_write_reg(hw, ST_LSM6DSVXHG_TGT0_ADD_ADDR,
					   config, sizeof(config));
	if (err < 0)
		return err;

	/* AuxSens = 3 + wr once */
	data = ST_LSM6DSVXHG_WRITE_ONCE_MASK | 3;
	return st_lsm6dsvxhg_shub_write_reg(hw,
					   ST_LSM6DSVXHG_CONTROLLER_CONFIG_ADDR,
					   &data, sizeof(data));
}
