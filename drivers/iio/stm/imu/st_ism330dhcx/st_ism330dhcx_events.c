...
/*
 * st_ism330dhcx_set_wake_up_thershold - set wake-up threshold in mg
 ...
 */
static int st_ism330dhcx_set_wake_up_thershold(struct st_ism330dhcx_hw *hw,
				    ...
				    int val, int val2)
{
	...
	case IIO_EV_TYPE_THRESH:
		...
		case IIO_EV_DIR_RISING:
			...
			switch (info) {
			case IIO_EV_INFO_VALUE:
				err = st_ism330dhcx_set_wake_up_thershold(hw,
									  val);
				break;
...
int st_ism330dhcx_update_threshold_events(struct st_ism330dhcx_hw *hw)
{
	...
	ret = st_ism330dhcx_set_wake_up_thershold(hw, hw->wk_th_mg);
	if (ret < 0)
		return ret;
...
int st_ism330dhcx_event_init(struct st_ism330dhcx_hw *hw)
{
	...
	err = st_ism330dhcx_set_wake_up_thershold(hw, 100);
	if (err < 0)
		return err;
