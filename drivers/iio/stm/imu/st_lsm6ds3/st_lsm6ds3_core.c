...
#define ST_LSM6DS3_FIFO_MODE_CONTINUOUS		0x06
...
static inline int st_lsm6ds3_enable_embedded_page_regs(struct lsm6ds3_data *cdata, bool enable)
{
	...
	return st_lsm6ds3_write_register(cdata,
				       ...
				       u8 reg_addr, u8 *data, int len)
{
	...
	err = st_lsm6ds3_write_register(cdata, reg_addr, len, data, false);
	if (err < 0)
...
static int lsm6ds3_set_watermark(struct lsm6ds3_data *cdata)
{
	...
	if (fifo_watermark != cdata->fifo_watermark) {
		...
		err = st_lsm6ds3_write_register(cdata,
						ST_LSM6DS3_FIFO_THR_L_ADDR, 2,
...
int st_lsm6ds3_set_fifo_mode(struct lsm6ds3_data *cdata, enum fifo_mode fm)
{
	...
	case BYPASS:
		...
		break;
	case CONTINUOUS:
		reg_value = ST_LSM6DS3_FIFO_MODE_CONTINUOUS | ST_LSM6DS3_FIFO_ODR_MAX;
		break;
	default:
		return -EINVAL;
	}

	err = st_lsm6ds3_write_register(cdata, ST_LSM6DS3_FIFO_MODE_ADDR,
					1, &reg_value, true);
	if (err < 0)
		return err;
							...
							u8 decimators[3])
{
	...
	return st_lsm6ds3_write_register(cdata, ST_LSM6DS3_FIFO_DECIMATOR_ADDR,
	...
	if (sdata->cdata->sensors_use_fifo > 0) {
		...
		if (fifo_conf_changed) {
			...
			if ((samples_in_pattern[0] > 0) || (samples_in_pattern[1] > 0) || (samples_in_pattern[2] > 0)) {
				err = st_lsm6ds3_set_fifo_mode(sdata->cdata, CONTINUOUS);
				if (err < 0)
					goto reenable_fifo_irq;
				...
				if (IS_ENABLED(CONFIG_ST_LSM6DS3_IIO_MASTER_SUPPORT)) {
					if (((temp_num_samples[2] > 0) &&
						...
						if (sdata->cdata->fifo_enable_timestamp > temp_last_timestamp[2]) {
							...
							for (n = 0; n < n_gen; n++) {
								temp_last_timestamp[2] += temp_deltatime;
								err = st_lsm6ds3_push_data_with_timestamp(sdata->cdata,
													  ST_MASK_ID_EXT0,
													  sdata->cdata->ext0_last_push,
													  temp_last_timestamp[2]);
								if (err < 0)
...
static int lsm6ds3_enable_accel(struct lsm6ds3_data *cdata, enum st_mask_id id, int min_odr)
{
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DS3_XL_DATA_INJECTION) &&
							...
							int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		err = st_lsm6ds3_read_register(sdata->cdata, ch->address,
...
static int st_lsm6ds3_init_sensor(struct lsm6ds3_data *cdata)
{
	...
	err = st_lsm6ds3_write_register(cdata, ST_LSM6DS3_RESET_ADDR, 1,
	...
	if (IS_ENABLED(CONFIG_ST_LSM6DS3_XL_DATA_INJECTION)) {
		...
		if (sdata->cdata->v_odr[sdata->sindex] != odr) {
			if ((sdata->sindex == ST_MASK_ID_ACCEL) &&
				...
				err = lsm6ds3_enable_accel(sdata->cdata,
							   ST_MASK_ID_ACCEL,
							   odr);
			else
				err = st_lsm6ds3_set_odr(sdata, odr, false);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	err = st_lsm6ds3_read_register(sdata->cdata, reg_addr,
				       1, &reg_status, true);
	if (err < 0) {
		mutex_unlock(&sdata->cdata->odr_lock);
		return err;
	...
	err = st_lsm6ds3_write_register(sdata->cdata, reg_addr,
	...
	for (i = 0; i < 20; i++) {
		err = st_lsm6ds3_read_register(sdata->cdata,
					       sdata->data_out_reg,
					       6, outdata, true);
		if (err < 0) {
			i--;
...
#if IS_ENABLED(CONFIG_ST_LSM6DS3_XL_DATA_INJECTION)
static ssize_t
st_lsm6ds3_sysfs_set_injection_mode(struct device *dev,
...
static ssize_t
st_lsm6ds3_sysfs_get_injection_mode(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct lsm6ds3_sensor_data *sdata = iio_priv(indio_dev);
...
static ssize_t
st_lsm6ds3_sysfs_upload_xl_data(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	int i;
...
static void st_lsm6ds3_injection_work(struct work_struct *work)
{
	...
	err = st_lsm6ds3_write_register(cdata, ST_LSM6DS3_INJECT_XL_X_ADDR,
...
static enum
hrtimer_restart st_lsm6ds3_injection_timer_func(struct hrtimer *timer)
{
	ktime_t now;
	struct lsm6ds3_data *cdata;
...
static ssize_t
st_lsm6ds3_sysfs_get_injection_sensors(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", "lsm6ds3_accel");
}
		     ...
		     char *name, struct regmap *regmap)
{
	...
	cdata->fifo_data = devm_kzalloc(dev, ST_LSM6DS3_MAX_FIFO_SIZE *
	...
	st_hrtimer_setup(&cdata->injection_timer, &st_lsm6ds3_injection_timer_func);
	spin_lock_init(&cdata->injection_spinlock);
#endif /* CONFIG_ST_LSM6DS3_XL_DATA_INJECTION */

	err = st_lsm6ds3_read_register(cdata, ST_LSM6DS3_WAI_ADDRESS,
				       1, &wai, true);
	if (err < 0) {
...
EXPORT_SYMBOL(st_lsm6ds3_probe);
...
static int __maybe_unused st_lsm6ds3_suspend(struct device *dev)
{
	...
	for (i = 0; i < ST_INDIO_DEV_NUM; i++) {
		...
		if (IS_ENABLED(CONFIG_ST_LSM6DS3_STEP_COUNTER_ON_DURING_SUSPEND)) {
...
const struct dev_pm_ops st_lsm6ds3_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(st_lsm6ds3_suspend, st_lsm6ds3_resume)
};
EXPORT_SYMBOL(st_lsm6ds3_pm_ops);
