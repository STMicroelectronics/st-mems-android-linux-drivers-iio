...
#define ST_LSM6DSOX_STEP_CHANNEL(addr)					\
{									\
	.type = IIO_STEPS,						\
	.address = addr,						\
	...
	.event_spec = st_lsm6dsox_step_events,				\
	.num_event_specs = ARRAY_SIZE(st_lsm6dsox_step_events),		\
}

static const struct iio_event_spec st_lsm6dsox_step_events[] = {
	{
		/* step detector */
...
static const struct iio_chan_spec st_lsm6dsox_step_channels[] = {
	ST_LSM6DSOX_STEP_CHANNEL(ST_LSM6DSOX_REG_STEP_COUNTER_L_ADDR),
...
static int st_lsm6dsox_reset_step_counter(struct iio_dev *iio_dev)
{
	...
	err = __st_lsm6dsox_write_with_mask(hw,
					    ...
					    ST_LSM6DSOX_REG_PEDO_RST_STEP_MASK,
					    1);
 ...
 */
static ssize_t
st_lsm6dsox_sysfs_reset_step_counter(struct device *dev,
 ...
 * @return  < 0 if error, 0 otherwise
 */
static int st_lsm6dsox_pedometer_enable(struct st_lsm6dsox_sensor *sensor,
					bool enable)
{
	struct st_lsm6dsox_hw *hw = sensor->hw;
	int err;
 ...
 */
static int
st_lsm6dsox_step_event_enable(struct st_lsm6dsox_sensor *sensor, bool enable)
{
	struct st_lsm6dsox_hw *hw = sensor->hw;
	int err;

	err = st_lsm6dsox_pedometer_enable(sensor, enable);
	if (err < 0)
		return err;
	...
	err = __st_lsm6dsox_write_with_mask(hw, hw->embfunc_irq_reg,
 ...
 * @return  < 0 if error, 0 otherwise
 */
static int st_lsm6dsox_signmot_enable(struct st_lsm6dsox_sensor *sensor,
				      bool enable)
{
	struct st_lsm6dsox_hw *hw = sensor->hw;
	int err;
 ...
 */
static int
st_lsm6dsox_signmot_event_enable(struct st_lsm6dsox_sensor *sensor, bool enable)
{
	struct st_lsm6dsox_hw *hw = sensor->hw;
	int err;

	err = st_lsm6dsox_signmot_enable(sensor, enable);
	if (err < 0)
		return err;
 ...
 */
static int st_lsm6dsox_read_event_step_config(struct iio_dev *iio_dev,
 ...
 */
static int st_lsm6dsox_write_event_step_config(struct iio_dev *iio_dev,
					       const struct iio_chan_spec *chan,
					       enum iio_event_type type,
					       enum iio_event_direction dir,
					       int state)
	...
	if (chan->type == IIO_STEPS) {
		...
		case IIO_EV_TYPE_CHANGE:
			...
			case IIO_EV_DIR_NONE:
				err = st_lsm6dsox_step_event_enable(sensor,
								    state);
				break;
			...
			case IIO_EV_DIR_RISING:
				err = st_lsm6dsox_signmot_event_enable(sensor,
								       state);
				break;
...
static int st_lsm6dsox_read_step_counter(struct st_lsm6dsox_sensor *sensor,
					 u8 addr, int *val)
{
	struct st_lsm6dsox_hw *hw = sensor->hw;
	__le16 data;
...
static int st_lsm6dsox_read_step_raw(struct iio_dev *iio_dev,
				     ...
				     int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_PROCESSED:
		...
		case IIO_STEPS:
			...
			ret = st_lsm6dsox_read_step_counter(sensor,
							    ch->address, val);
			iio_device_release_direct_mode(iio_dev);
			break;
		default:
...
static int st_lsm6dsox_write_step_raw(struct iio_dev *iio_dev,
				      ...
				      int val, int val2, long mask)
{
	...
	if (mask == IIO_CHAN_INFO_ENABLE)
		return st_lsm6dsox_step_enable(sensor, val);
...
static struct attribute *st_lsm6dsox_step_attributes[] = {
	&iio_dev_attr_reset_counter.dev_attr.attr,
	&iio_dev_attr_module_id.dev_attr.attr,
	NULL,
};

static const struct attribute_group st_lsm6dsox_step_attribute_group = {
	.attrs = st_lsm6dsox_step_attributes,
};

static const struct iio_info st_lsm6dsox_step_info = {
	.attrs = &st_lsm6dsox_step_attribute_group,
	.read_raw = st_lsm6dsox_read_step_raw,
	.write_raw = st_lsm6dsox_write_step_raw,
	.read_event_config = st_lsm6dsox_read_event_step_config,
	.write_event_config = st_lsm6dsox_write_event_step_config,
};

/**
 * st_lsm6dsox_embfunc_handler_thread() - Bottom handler for embedded function
 *
...
int st_lsm6dsox_embfunc_handler_thread(struct st_lsm6dsox_hw *hw)
{
	...
	if (!(hw->enable_mask & BIT(ST_LSM6DSOX_ID_STEP_COUNTER)))
		return IRQ_HANDLED;
...
static struct
iio_dev *st_lsm6dsox_alloc_step_iiodev(struct st_lsm6dsox_hw *hw,
				       enum st_lsm6dsox_sensor_id id)
{
	...
	iio_dev->channels = st_lsm6dsox_step_channels;
	iio_dev->num_channels =
			ARRAY_SIZE(st_lsm6dsox_step_channels);
	scnprintf(sensor->name, sizeof(sensor->name), "%s_step", hw->dev_name);
	iio_dev->info = &st_lsm6dsox_step_info;

	/* request acc ODR to works properly */
	sensor->odr = ST_LSM6DSOX_MIN_ODR_IN_EMB_FUNC;
	sensor->uodr = 0;
...
static int st_lsm6dsox_embedded_function_init(struct st_lsm6dsox_hw *hw)
{
	...
	__st_lsm6dsox_write_with_mask(hw, ST_LSM6DSOX_PAGE_RW_ADDR,
				      ST_LSM6DSOX_REG_EMB_FUNC_LIR_MASK, 1);
	st_lsm6dsox_set_page_access(hw, false, ST_LSM6DSOX_REG_FUNC_CFG_MASK);

unlock:
	/* enable embedded function interrupts enable */
	err  = __st_lsm6dsox_write_with_mask(hw, hw->embfunc_pg0_irq_reg,
...
/**
 * st_lsm6dsox_embfunc_probe() - Enable embedded functionalities
 *
 * @hw: ST IMU MEMS hw instance.
 *
...
int st_lsm6dsox_embfunc_probe(struct st_lsm6dsox_hw *hw)
{
	...
	id = ST_LSM6DSOX_ID_STEP_COUNTER;
	hw->iio_devs[id] = st_lsm6dsox_alloc_step_iiodev(hw, id);
	if (!hw->iio_devs[id])
		return -ENOMEM;
