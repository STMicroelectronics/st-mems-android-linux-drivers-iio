 ...
 */
static const struct st_lsm6dsox_ext_dev_settings {
	u8 i2c_addr[2];
	u8 wai_addr;
	u8 wai_val;
	...
	},
	{
		...
		.ext_available_scan_masks = { 0x1, 0x0 },
		.ext_channels[0] = ST_LSM6DSOX_DATA_CHANNEL(IIO_PRESSURE,
							 0x28, 0, IIO_NO_MOD, 0,
							 24, 32, 'u', NULL),
		.ext_channels[1] = ST_LSM6DSOX_EVENT_CHANNEL(IIO_PRESSURE,
							     flush),
 ...
 * @return  0 if OK, < 0 if ERROR
 */
int st_lsm6dsox_shub_read(struct st_lsm6dsox_sensor *sensor, u8 addr,
			  u8 *data, int len)
{
