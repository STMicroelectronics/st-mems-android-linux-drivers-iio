...
static const struct st_asm330lhhx_lpf_discard_table_t {
	...
	[ST_ASM330LHHX_ID_GYRO] = {
		 ...
		 */
		.samples_to_discard = {
			2, 2 + 1, 3 + 2, 3 + 4, 3 + 7, 3 + 15, 3 + 30, 3 + 58
		},
...
static const struct st_asm330lhhx_settings st_asm330lhhx_sensor_settings[] = {
	...
	{
		.id = {
			.hw_id = ST_ASM330LHHG1_ID,
			.name = ST_ASM330LHHG1_DEV_NAME,
		},
		.st_shub_probe = true,
		.st_power_mode = true,
	},
	{
		.id = {
			.hw_id = ST_ASM330LHBG1_ID,
			.name = ST_ASM330LHBG1_DEV_NAME,
		},
		.st_mlc_probe = true,
		.st_power_mode = true,
	},
...
static int
st_asm330lhhx_set_selftest(struct st_asm330lhhx_sensor *sensor, int index)
{
	u8 mode, mask;
...
static int st_asm330lhhx_check_whoami(struct st_asm330lhhx_hw *hw, int id)
{
	int err, i;
	int data;
					...
					int val, int delay)
{
	...
	if (fsm_running || mlc_running ||
		...
		ret = regmap_update_bits(hw->regmap,
					 ...
					 FIELD_PREP(ST_ASM330LHHX_FSM_EN_MASK,
						    0));
		if (ret < 0)
			goto unlock_page;
					  ...
					  int odr)
{
	...
	case ST_ASM330LHHX_ID_ACC:
		odr_div_index =
			    st_asm330lhhx_get_odr_divider_index(hw->xl_odr_div);
		if (odr_div_index < 0) {
			ret = odr_div_index;
				      ...
				      u8 addr, int *val)
{
	...
	err = st_asm330lhhx_read_locked(hw, addr, &data, sizeof(data));
	if (err < 0)
		return err;
...
static ssize_t __maybe_unused
st_asm330lhhx_get_wakeup_status(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct iio_dev *iio_dev = dev_to_iio_dev(dev);
	struct st_asm330lhhx_sensor *sensor = iio_priv(iio_dev);
...
static int st_asm330lhhx_init_g_filters(struct st_asm330lhhx_hw *hw)
{
	...
	if (err < 0) {
		...
		return regmap_update_bits(hw->regmap,
				...
				ST_ASM330LHHX_REG_LPF1_SEL_G_MASK,
				FIELD_PREP(ST_ASM330LHHX_REG_LPF1_SEL_G_MASK,
					   0));
	}
