...
static const struct st_lps22hh_odr_table_t st_lps22hh_odr_table = {
	.addr = 0x10,
	.mask = 0x70,
...
{
	unsigned int data = ST_LPS22HH_SHIFT_VAL(val, mask);
	int ret;
...
static int st_lps22hh_check_whoami(struct st_lps22hh_hw *hw)
{
	...
	err = regmap_read(hw->regmap, ST_LPS22HH_WHO_AM_I_ADDR, &data);
	if (err < 0) {
...
static int st_lps22hh_init_sensors(struct st_lps22hh_hw *hw)
{
	int err;
